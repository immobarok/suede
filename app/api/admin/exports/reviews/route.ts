...
export async function GET() {
  const data = await db.select().from(reviews).limit(10000);
    ...
    .map((item) => {
      return `
        <tr>
          ...
          <td>${escapeHtml(item.brandId)}</td>
          <td>${escapeHtml(item.productName)}</td>
          <td>${escapeHtml(item.category)}</td>
          <td>${escapeHtml(item.reviewText)}</td>
          <td>${escapeHtml(item.isPublished)}</td>
  ...
  <html>
    ...
    <body>
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        ...
        <div>
          <h2 style="margin:0">Reviews Export</h2>
  ...
  return new Response(html, {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="reviews.xls"',
    },
  });
}
