...
export async function GET() {
  const data = await db.select().from(platformFeedback).limit(10000);
    ...
    .map((item) => {
      return `
        <tr>
          ...
          <td>${escapeHtml(item.suggestionText)}</td>
  ...
  <html>
    ...
    <body>
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        ...
        <div>
          <h2 style="margin:0">Platform Feedback Export</h2>
  ...
  return new Response(html, {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="platform-feedback.xls"',
    },
  });
}
