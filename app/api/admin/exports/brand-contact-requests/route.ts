...
import { db } from "@/db";
import { brandContactRequests } from "@/db/schema";
...
export async function GET() {
  const data = await db.select().from(brandContactRequests).limit(10000);
  ...
  <html>
    ...
    <body>
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        ...
        <div>
          <h2 style="margin:0">Brand Contact Requests Export</h2>
  ...
  return new Response(html, {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="brand-contact-requests.xls"',
    },
  });
}
