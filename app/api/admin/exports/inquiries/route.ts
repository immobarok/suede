...
export async function GET() {
  const data = await db.select().from(reviewRequests).limit(10000);
    ...
    .map((item) => {
      return `
        <tr>
          ...
          <td>${escapeHtml(item.specificQuestions)}</td>
  ...
  <html>
    ...
    <body>
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        ...
        <div>
          <h2 style="margin:0">Inquiries Export</h2>
  ...
  return new Response(html, {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="inquiries.xls"',
    },
  });
}
